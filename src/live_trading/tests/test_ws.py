...
SLACK_CHANNEL = os.getenv('SLACK_CHANNEL', '#crypto-logs')
...
def update_data_for_file(file_path, symbol):
    try:
        ...
        info_msg = f"[{file_path}] 현재 시간: {now.strftime('%Y.%m.%d %H:%M')}, 마지막 데이터 시간: {last_timestamp.strftime('%Y.%m.%d %H:%M') if not pd.isna(last_timestamp) else '없음'}, 누락된 분: {missing_minutes}"
        logger.info(info_msg)
        send_slack_message(info_msg, level='INFO')
