...
category_path = os.path.join(result_path_strategy1, "defi")
...
def run_backtest_for_row(row_index, base_path, result_path, coint_file_path, entry_z_score, exit_z_score, stop_z_score):
    try:
        ...
        data1_path = os.path.join(base_path, f"{crypto1}USDT_1m.csv")
        data2_path = os.path.join(base_path, f"{crypto2}USDT_1m.csv")
...
# if __name__ == "__main__":
#     run_backtest_for_all_rows(data_path,category_path,coint_file_path,1.2,0.3,4)

if __name__ =="__main__":
    run_backtest_for_row(1,data_path,category_path,coint_file_path,1.3,0.2,4)